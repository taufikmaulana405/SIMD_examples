...
//
// 0.0 = uniform particle density over the disk area.
// 1.0 = fully use the center-biased distribution below.
const CENTER_CONCENTRATION_STRENGTH: f32 = 0.75;

// Controls the center-biased radial distribution.
//
// 0.5 = uniform density over the disk area.
// >0.5 = increasingly concentrated near the center.
// <0.5 = increasingly concentrated near the outer edge.
const CENTER_CONCENTRATION_EXPONENT: f32 = 1.40;

// Relative initial rotation weights. They do not need to add up to 1.0.
const CLOCKWISE_ROTATION_WEIGHT: f32 = 1.0;
const COUNTERCLOCKWISE_ROTATION_WEIGHT: f32 = 0.00;

// Particles near the center start slower; particles near the outer edge
// approach INITIAL_TANGENTIAL_SPEED_MAX.
const INITIAL_TANGENTIAL_SPEED_MIN: f32 = 6.0;
const INITIAL_TANGENTIAL_SPEED_MAX: f32 = 20.0;

// Shape of the tangential-speed profile versus normalized radius.
//
// 0.50 = square-root profile.
// 1.00 = linear profile.
// >1.0 = inner region remains slower for longer.
// <1.0 = speed rises more quickly near the center.
const INITIAL_TANGENTIAL_SPEED_RADIUS_EXPONENT: f32 = 0.50;

// Random inward/outward component. Set to 0.0 for purely tangential motion.
...
// Full trail width in screen pixels, not radius.
const TRAIL_MIN_WIDTH_PIXELS: f32 = 1.0;
const TRAIL_MAX_WIDTH_PIXELS: f32 = 2.0;

// Multiplies the object's world-space diameter before applying the limits.
const TRAIL_WIDTH_SCALE: f32 = 1.0;

// A particle starts turning yellow at 25% of total system mass.
const YELLOW_TRANSITION_START_MASS_FRACTION: f32 = 0.25;

// At 45%, it fully matches the glowing central-body appearance.
...
// Trail points are independent of particles. If a particle merges, its old
// trail remains in this collection and fades normally.
struct TrailPoint {
    position: Vec2,
    radius: f32,
    brightness: f32,
...
fn window_configuration() -> Conf {
    Conf {
        window_title: "Non-SIMD Rotating Particle Cloud".to_owned(),
        window_width: 1280,
        window_height: 800,
        high_dpi: true,
...
fn center_of_mass(particles: &[Particle]) -> Vec2 {
    ...
    let weighted_position = particles.iter().fold(
        Vec2::ZERO,
        |sum, particle| sum + particle.position * particle.mass,
    );

    weighted_position / system_mass
}

// Shift the initial center of mass to the origin and remove the cloud's bulk
// velocity so the isolated system does not immediately drift off-screen.
...
fn dominant_body_transition(mass_fraction: f32) -> f32 {
    ...
    // Smoothstep avoids an abrupt visual change at either threshold.
...
fn sample_spawn_distance() -> f32 {
    ...
    // An exponent greater than 0.5 biases samples toward the center.
...
fn spawn_particle(clockwise: bool) -> Particle {
    ...
    let radial_direction =
        vec2(position_angle.cos(), position_angle.sin());

    // Screen-space Y increases downward. At the right side of the disk,
    // clockwise motion points downward, so this is the clockwise tangent.
    ...
    let radial_speed: f32 = gen_range(
...
fn validate_parameters() {
    ...
    assert!(
        TRAIL_MIN_WIDTH_PIXELS > 0.0
            && TRAIL_MAX_WIDTH_PIXELS >= TRAIL_MIN_WIDTH_PIXELS,
        "Trail widths must satisfy 0 < MIN <= MAX",
    );
...
fn create_particles() -> Vec<Particle> {
    ...
    // This creates an exact configured split rather than a merely probable one.
    let clockwise_count =
...
// Exact scalar all-pairs gravity. Each pair is processed only once, and the
// equal-and-opposite interaction updates both particles' accelerations.
...
fn find_collision(
    ...
    for second_index in (first_index + 1)..particles.len() {
        ...
        let collision_distance =
            first_radius + particle_radius(second.mass);

        if second
            .position
            .distance_squared(first.position)
...
fn merge_pair(
    ...
    let combined_brightness =
        (first.brightness * first.mass
            + second.brightness * second.mass)
            / combined_mass;
    ...
    // Particle order is irrelevant, so this avoids shifting later elements.
...
fn draw_trails(
    ...
    for trail in trails {
        let screen_position =
            screen_center + trail.position * zoom;

        // Convert stored world-space size to screen-space width, then cap it
        // so very large merged objects do not leave excessively wide trails.
        let raw_trail_width =
        ...
        let fade =
            0.5_f32.powf(age / TRAIL_HALF_LIFE_SECONDS);
...
fn draw_particles(
    ...
    for particle in particles {
        let screen_position =
            screen_center + particle.position * zoom;
        ...
        let star_transition =
            dominant_body_transition(mass_fraction);

        // At full transition, the outer glow reaches 2.7 radii.
        ...
        let mass_brightness =
            (particle.mass / MAX_PARTICLE_MASS)
                .sqrt()
                .clamp(0.0, 1.0);

        let normal_color = Color::new(
            (particle.brightness * 0.65
                + mass_brightness * 0.25)
                .min(1.0),
            (particle.brightness * 0.75
                + mass_brightness * 0.20)
                .min(1.0),
            particle.brightness.min(1.0),
            0.95,
        );

        let central_body_color =
        ...
        // Glow layers appear gradually between the two mass thresholds.
        ...
        // At 50% mass, this matches the original central-body highlight.
        if star_transition > 0.0 {
            draw_circle(
...
fn draw_interface(
    ...
    let first_line = format!(
        "{} particles | {} | Initial rotation: {:.0}% CW / {:.0}% CCW | Scalar physics | FPS: {}",
        particles.len(),
        simulation_state,
        clockwise_percent,
...
async fn main() {
    ...
    loop {
        ...
        // T controls visibility only. Existing trail data is not cleared.
        ...
        if wheel_y != 0.0 {
            ...
            // Trails use world coordinates, so zooming does not clear them.
        }
        ...
        if !paused {
            ...
            if completed_steps == MAX_PHYSICS_STEPS_PER_FRAME {
                // Avoid an ever-growing physics backlog after a slow frame.
                accumulated_time = 0.0;
            }

            // Trails age and continue recording even while hidden.
            trail_clock += frame_time;
            remove_expired_trails(&mut trails, trail_clock);

            trail_sample_accumulator += frame_time;

            while trail_sample_accumulator
