...
// making it convenient to load sixteen values into one AVX-512 vector.
struct Particles {
    x: Vec<f32>,
    y: Vec<f32>,
    velocity_x: Vec<f32>,
...
fn window_configuration() -> Conf {
    Conf {
        window_title: "AVX-512 Rotating Particle Cloud".to_owned(),
        window_width: 1280,
        window_height: 800,
        high_dpi: true,
...
unsafe fn load_f32x16(values: &[f32], start: usize) -> __m512 {
    debug_assert!(start + SIMD_LANES <= values.len());
    unsafe { _mm512_loadu_ps(values.as_ptr().add(start)) }
}

#[inline]
#[target_feature(enable = "avx512f")]
unsafe fn store_f32x16(values: &mut [f32], start: usize, vector: __m512) {
    debug_assert!(start + SIMD_LANES <= values.len());
    unsafe { _mm512_storeu_ps(values.as_mut_ptr().add(start), vector) };
}

// Exact all-pairs gravity. For each fixed first particle, interactions with
// sixteen second particles are evaluated together in one 512-bit register.
#[target_feature(enable = "avx512f")]
unsafe fn calculate_accelerations_avx512(
    particles: &Particles,
    ...
    let gravitational_constant = _mm512_set1_ps(GRAVITATIONAL_CONSTANT);
    let softening_squared =
        _mm512_set1_ps(GRAVITY_SOFTENING * GRAVITY_SOFTENING);
    ...
    for first_index in 0..particle_count {
        ...
        let first_x = _mm512_set1_ps(first_x_scalar);
        let first_y = _mm512_set1_ps(first_y_scalar);
        let first_mass = _mm512_set1_ps(first_mass_scalar);
        ...
        while second_index + SIMD_LANES <= particle_count {
            let second_x =
                unsafe { load_f32x16(&particles.x, second_index) };
            let second_y =
                unsafe { load_f32x16(&particles.y, second_index) };
            let second_mass =
                unsafe { load_f32x16(&particles.mass, second_index) };

            let displacement_x = _mm512_sub_ps(second_x, first_x);
            let displacement_y = _mm512_sub_ps(second_y, first_y);

            let distance_squared = _mm512_fmadd_ps(
                displacement_x,
                displacement_x,
                _mm512_fmadd_ps(
                    displacement_y,
            ...
            let inverse_distance =
                _mm512_div_ps(one, _mm512_sqrt_ps(distance_squared));
            let inverse_distance_squared =
                _mm512_mul_ps(inverse_distance, inverse_distance);
            let inverse_distance_cubed =
                _mm512_mul_ps(inverse_distance_squared, inverse_distance);

            let common_factor = _mm512_mul_ps(
                gravitational_constant,
                inverse_distance_cubed,
            );

            // Acceleration of the first particle from sixteen partners.
            let second_weighted_factor =
                _mm512_mul_ps(common_factor, second_mass);

            first_acceleration_x = _mm512_fmadd_ps(
                displacement_x,
                second_weighted_factor,
                first_acceleration_x,
            );

            first_acceleration_y = _mm512_fmadd_ps(
                displacement_y,
                second_weighted_factor,
            ...
            // sixteen second particles.
            let first_weighted_factor =
                _mm512_mul_ps(common_factor, first_mass);

            let updated_second_acceleration_x = _mm512_sub_ps(
                unsafe { load_f32x16(acceleration_x, second_index) },
                _mm512_mul_ps(displacement_x, first_weighted_factor),
            );

            let updated_second_acceleration_y = _mm512_sub_ps(
                unsafe { load_f32x16(acceleration_y, second_index) },
                _mm512_mul_ps(displacement_y, first_weighted_factor),
            );
        ...
        // Scalar remainder for the final zero to fifteen partners.
        ...
        acceleration_x[first_index] +=
            _mm512_reduce_add_ps(first_acceleration_x)
                + scalar_first_acceleration_x;
        acceleration_y[first_index] +=
            _mm512_reduce_add_ps(first_acceleration_y)
                + scalar_first_acceleration_y;
    }
}

#[target_feature(enable = "avx512f")]
unsafe fn integrate_positions_avx512(
    ...
    let particle_count = particles.len();
    let half_dt = _mm512_set1_ps(dt * 0.5);
    let full_dt = _mm512_set1_ps(dt);

    let mut index = 0;

    while index + SIMD_LANES <= particle_count {
        let velocity_x = _mm512_fmadd_ps(
            unsafe { load_f32x16(acceleration_x, index) },
            half_dt,
            unsafe { load_f32x16(&particles.velocity_x, index) },
        );

        let velocity_y = _mm512_fmadd_ps(
            unsafe { load_f32x16(acceleration_y, index) },
            half_dt,
            unsafe { load_f32x16(&particles.velocity_y, index) },
        );

        let x = _mm512_fmadd_ps(
            velocity_x,
            full_dt,
            unsafe { load_f32x16(&particles.x, index) },
        );

        let y = _mm512_fmadd_ps(
            velocity_y,
            full_dt,
            unsafe { load_f32x16(&particles.y, index) },
        );

        unsafe {
            store_f32x16(&mut particles.velocity_x, index, velocity_x);
            store_f32x16(&mut particles.velocity_y, index, velocity_y);
            store_f32x16(&mut particles.x, index, x);
            store_f32x16(&mut particles.y, index, y);
        }
...
#[target_feature(enable = "avx512f")]
unsafe fn finish_velocity_update_avx512(
    particles: &mut Particles,
    acceleration_x: &[f32],
    acceleration_y: &[f32],
    dt: f32,
...
// Tests sixteen possible collision partners simultaneously and returns the
// first one that overlaps the selected particle.
#[target_feature(enable = "avx512f")]
unsafe fn find_collision_avx512(
    particles: &Particles,
    first_index: usize,
) -> Option<usize> {
    let particle_count = particles.len();

    let first_x = _mm512_set1_ps(particles.x[first_index]);
    let first_y = _mm512_set1_ps(particles.y[first_index]);
    let first_radius =
        _mm512_set1_ps(particle_radius(particles.mass[first_index]));
    let radius_scale = _mm512_set1_ps(PARTICLE_RADIUS_SCALE);

    let mut second_index = first_index + 1;

    while second_index + SIMD_LANES <= particle_count {
        let displacement_x = _mm512_sub_ps(
            unsafe { load_f32x16(&particles.x, second_index) },
            first_x,
        );
        let displacement_y = _mm512_sub_ps(
            unsafe { load_f32x16(&particles.y, second_index) },
            first_y,
        );
        ...
        let distance_squared = _mm512_fmadd_ps(
            displacement_x,
            displacement_x,
            _mm512_mul_ps(displacement_y, displacement_y),
        );

        let second_radius = _mm512_mul_ps(
            _mm512_sqrt_ps(unsafe {
                load_f32x16(&particles.mass, second_index)
            }),
            radius_scale,
        );

        let collision_distance = _mm512_add_ps(first_radius, second_radius);
        let collision_distance_squared =
            _mm512_mul_ps(collision_distance, collision_distance);

        let collision_mask = _mm512_cmp_ps_mask::<_CMP_LE_OQ>(
            distance_squared,
...
#[target_feature(enable = "avx512f")]
unsafe fn merge_colliding_particles_avx512(particles: &mut Particles) {
    ...
    while first_index < particles.len() {
        ...
        while let Some(second_index) =
            unsafe { find_collision_avx512(particles, first_index) }
        {
...
#[target_feature(enable = "avx512f")]
unsafe fn update_system_avx512_impl(
    particles: &mut Particles,
    acceleration_x: &mut Vec<f32>,
    acceleration_y: &mut Vec<f32>,
    dt: f32,
    ...
    unsafe {
        calculate_accelerations_avx512(
            particles,
            acceleration_x,
            acceleration_y,
        );
    ...
    unsafe { merge_colliding_particles_avx512(particles) };
...
fn draw_interface(
    ...
    let first_line = format!(
        "{} particles | {} | Initial rotation: {:.0}% CW / {:.0}% CCW | AVX-512: 16 x f32 | FPS: {}",
        particles.len(),
        simulation_state,
        clockwise_percent,
...
async fn main() {
    ...
    assert!(
        ...
        "This build requires an x86-64 CPU and operating system with AVX-512F support",
    );
